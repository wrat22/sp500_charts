...
from app import app
from utils import calculate_pct_change, calculate_last_week
from models import (
...
def test_get_stock_data_valid(client, sample_data, mocker):
    mocker.patch(
        'routes.get_stock_companies_from_db',
...
def test_get_stock_data_invalid_company(client, mocker):
    mocker.patch(
        "routes.get_stock_companies_from_db", return_value=pd.DataFrame(
            {"ticker": ["AAPL", "GOOGL"], "name": ["Apple", "Google"]}
        )
    )
    mocker.patch("routes.get_stock_values_from_db", return_value=pd.DataFrame())

    response = client.post("/get_stock_data", data={"ticker": "InvalidCompany", "range": "all"})
    assert response.status_code == 200
    json_data = response.get_json()
...
def test_get_stock_data_no_ticker_data(client, mocker):
    ...
    mocker.patch(
        "routes.get_stock_values_from_db", return_value=pd.DataFrame(
            {"ticker": [], "date": [], "close": []}
        )
    )
...
def test_get_stock_data_invalid_ticker_format(client):
    response = client.post("/get_stock_data", data={"ticker": 12345, "range": "all"})
    json_data = response.get_json()
    assert "error" in json_data


def test_get_stock_data_long_ticker(client):
    response = client.post("/get_stock_data", data={"ticker": "A" * 256, "range": "all"})
    json_data = response.get_json()
    assert "error" in json_data
